...
import { CheckCircle2, Package, Truck, MapPin, Copy, Check, ShoppingBag } from "lucide-react";
...
export default function OrderConfirmationPage() {
  ...
  // Snapshot order on mount, then clear the cart
  ...
  }, [items, order, totalPrice, clearCart]);
  ...
  // Empty state — no order and no items
  if (!order && items.length === 0) {
    return (
      <div className="bg-white min-h-screen">
        <PageHeader title="Order Confirmation" subtitle="Your order details" badge="Thank You" />
        <section className="py-20">
          <div className="max-w-lg mx-auto px-4 text-center">
            <ShoppingBag size={64} className="mx-auto mb-6 text-text-secondary opacity-30" />
            <h2 className="text-2xl font-bold font-jost mb-4 text-foreground">No order found</h2>
            <p className="text-text-secondary mb-8">
              It looks like you haven&apos;t placed an order yet, or you&apos;ve already viewed your confirmation.
            </p>
  ...
  return (
    <div className="bg-white min-h-screen">
      <PageHeader title="Order Confirmed" subtitle="Thank you for your purchase!" badge="Success" />
      ...
      <section className="py-12 md:py-16">
        <div className="max-w-[90%] mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="mb-10 bg-gradient-to-r from-[#1a3014]/5 to-[#a9cb5b]/10 border border-[#a9cb5b]/20 rounded-2xl p-8 text-center">
            ...
            <h2 className="text-3xl font-bold font-jost text-[#1a3014] mb-2">Thank You for Your Order!</h2>
            ...
            <div className="mt-6 inline-flex items-center gap-3 bg-white border border-border rounded-xl px-6 py-3 shadow-sm">
              <span className="text-[10px] font-bold uppercase tracking-widest text-text-secondary">Order No.</span>
              <span className="text-lg font-bold font-jost text-[#1a3014] tracking-wider">{order.orderId}</span>
              <button
                onClick={handleCopy}
                className="w-8 h-8 rounded-lg bg-[#f8f9fa] flex items-center justify-center hover:bg-[#a9cb5b]/10 transition-colors"
                title="Copy order number"
              >
                {copied ? <Check size={14} className="text-[#a9cb5b]" /> : <Copy size={14} className="text-text-secondary" />}
              </button>
            </div>
          </div>
              ...
              {steps.map((step, idx) => (
                <div key={step.label} className="flex items-center flex-1">
                  <div className="flex flex-col items-center">
                    <div className={`w-12 h-12 rounded-full flex items-center justify-center transition-all ${step.active ? "bg-[#a9cb5b] text-white shadow-lg shadow-[#a9cb5b]/30" : "bg-white border-2 border-border text-text-secondary"}`}>
                      <step.icon size={20} />
                    </div>
                    <span className={`text-[10px] font-bold uppercase tracking-widest mt-2 ${step.active ? "text-[#1a3014]" : "text-text-secondary"}`}>
                      {step.label}
                    </span>
                  </div>
                  {idx < steps.length - 1 && (
                    <div className={`flex-1 h-0.5 mx-3 mt-[-20px] ${idx === 0 ? "bg-[#a9cb5b]" : "bg-border"}`} />
                  )}
                </div>
              ))}
            </div>
            ...
            <div className="lg:col-span-7">
              <div className="bg-[#f8f9fa] border border-border rounded-xl p-6 sm:p-8">
                ...
                <div className="grid grid-cols-2 gap-6 mb-8">
                  <div>
                    <span className="text-[10px] font-bold uppercase tracking-widest text-text-secondary block mb-1">Order Number</span>
                    <span className="text-sm font-bold text-foreground">{order.orderId}</span>
                  </div>
                  <div>
                    <span className="text-[10px] font-bold uppercase tracking-widest text-text-secondary block mb-1">Date</span>
                    <span className="text-sm font-bold text-foreground">{order.date}</span>
                  </div>
                  <div>
                    <span className="text-[10px] font-bold uppercase tracking-widest text-text-secondary block mb-1">Payment Method</span>
                    <span className="text-sm font-bold text-foreground">Direct bank transfer</span>
                  </div>
                  <div>
                    <span className="text-[10px] font-bold uppercase tracking-widest text-text-secondary block mb-1">Status</span>
              ...
              <div className="bg-[#f8f9fa] border border-border rounded-xl p-6 sm:p-8 sticky top-24">
                ...
                <div className="space-y-4">
                  <div className="flex justify-between items-center">
                    ...
                    <span className="text-sm font-bold text-foreground">{formatPrice(order.subtotal)}</span>
                  </div>
                  <div className="flex justify-between items-center">
                  ...
                  <div className="flex justify-between items-center py-4 border-t-2 border-border">
                    ...
                    <span className="text-2xl font-bold text-[#a9cb5b]">{formatPrice(order.total)}</span>
                  </div>
                </div>

                {/* Bank Transfer Details */}
                <div className="mt-6 p-5 bg-white rounded-xl border border-border">
                  <h4 className="text-[10px] font-bold uppercase tracking-widest text-text-secondary mb-4">Bank Transfer Details</h4>
                  <div className="space-y-3 text-sm">
                    <div className="flex justify-between">
                      <span className="text-text-secondary">Bank</span>
                      <span className="font-bold text-foreground">Barclays</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-text-secondary">Account Name</span>
                      <span className="font-bold text-foreground">AtyrePrint Ltd</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-text-secondary">Sort Code</span>
                      <span className="font-bold text-foreground">20-45-67</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-text-secondary">Account No.</span>
                      <span className="font-bold text-foreground">12345678</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-text-secondary">Reference</span>
                      <span className="font-bold text-[#a9cb5b]">{order.orderId}</span>
                    </div>
                  </div>
                </div>
