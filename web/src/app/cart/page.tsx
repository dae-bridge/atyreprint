...
import { Minus, Plus, X, ShoppingBag, Heart, RefreshCw, Gift, ShieldCheck, Truck } from "lucide-react";
 ...
 */
const BenefitBox = ({ icon: Icon, title, description }: { icon: any, title: string, description: string }) => (
  <div className="flex flex-col items-center text-center p-6 bg-[#f8f9fa] rounded-lg transition-all hover:shadow-md border border-transparent hover:border-[#a9cb5b]/20">
    ...
    <h3 className="font-bold text-foreground mb-1 font-jost uppercase tracking-wider text-sm">{title}</h3>
...
export default function CartPage() {
  ...
  const [shippingMethod, setShippingMethod] = useState<"flat" | "local">("flat");
  ...
  return (
    <div className="bg-white min-h-screen">
      <PageHeader
        title="Shopping Cart"
        subtitle={count > 0 ? `You have ${count} item${count !== 1 ? "s" : ""} in your cart` : undefined}
        badge="Your Bag"
      />
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          {items.length === 0 ? (
            <div className="text-center py-20 bg-surface rounded-2xl border border-dashed border-border-light">
              ...
              <h2 className="text-2xl font-bold text-foreground mb-3 font-jost uppercase tracking-widest">Your cart is empty</h2>
              <p className="text-text-secondary mb-10 max-w-md mx-auto">
                Looks like you haven't added any AtyrePrint items yet. Explore our collections and find your style!
              </p>
              <Link
                href="/shop"
            ...
            <div className="grid lg:grid-cols-12 gap-10 items-start">
              ...
              <div className="lg:col-span-8 space-y-8">
                ...
                <div className="bg-[#f1f8e9] border border-[#a9cb5b]/30 rounded-lg p-4 flex flex-col sm:flex-row items-center justify-between gap-4">
                  <div className="flex items-center gap-3">
                    ...
                    <p className="text-sm font-medium text-[#1a3014]">
                      Use <span className="font-bold">ATYREFREE</span> coupon code to get 10% off on your first order!
                    </p>
                  </div>
                  ...
                  <table className="w-full border-collapse">
                    <thead>
                      <tr className="border-b-2 border-border text-left">
                        <th className="pb-4 font-bold font-jost uppercase tracking-wider text-xs">Product</th>
                        <th className="pb-4 font-bold font-jost uppercase tracking-wider text-xs text-center">Price</th>
                        <th className="pb-4 font-bold font-jost uppercase tracking-wider text-xs text-center">Quantity</th>
                        <th className="pb-4 font-bold font-jost uppercase tracking-wider text-xs text-right">Subtotal</th>
                      </tr>
                    </thead>
                    <tbody>
                      {items.map((item) => (
                        <tr key={`${item.productId}-${item.color}-${JSON.stringify(item.variants)}`} className="border-b border-border-light group">
                          <td className="py-6">
                            <div className="flex items-center gap-5">
                              ...
                              <div className="min-w-0">
                                ...
                                <p className="text-xs text-text-secondary mt-1">
                                  {item.color} {Object.values(item.variants).filter(Boolean).length > 0 && ` / ${Object.values(item.variants).filter(Boolean).join(" / ")}`}
                                </p>
                              </div>
                            </div>
                          </td>
                ...
                <div className="md:hidden space-y-4">
                  {items.map((item) => (
                    <div key={`${item.productId}-${item.color}-${JSON.stringify(item.variants)}`} className="p-4 border border-border rounded-xl">
                      <div className="flex gap-4 mb-4">
                        <div className="relative w-20 h-20 bg-[#f8f9fa] rounded overflow-hidden flex-shrink-0">
                          <Image src={item.image} alt={item.name} fill className="object-contain p-2" />
                        </div>
                        <div className="flex-1 min-w-0">
                          <Link href={`/shop/product/${item.slug}`} className="font-bold text-foreground block truncate">
                            {item.name}
                          </Link>
                          <p className="text-xs text-text-secondary mt-1">{item.color}</p>
                          <p className="mt-2 font-bold text-[#a9cb5b]">{formatPrice(item.price)}</p>
                        </div>
                        <button onClick={() => removeItem(item.productId, item.color, item.variants)} className="text-text-muted h-fit"><X size={18} /></button>
                      </div>
                      <div className="flex items-center justify-between pt-4 border-t border-border-light">
                        <div className="flex items-center border border-border rounded h-9 w-28">
                          <button onClick={() => updateQuantity(item.productId, item.color, item.variants, item.quantity - 1)} className="flex-1 flex items-center justify-center"><Minus size={12} /></button>
                          <span className="w-8 text-center text-sm font-bold">{item.quantity}</span>
                          <button onClick={() => updateQuantity(item.productId, item.color, item.variants, item.quantity + 1)} className="flex-1 flex items-center justify-center"><Plus size={12} /></button>
                        </div>
                ...
                <div className="mb-6 p-6 border-2 border-dashed border-[#a9cb5b]/30 rounded-xl bg-white text-center relative overflow-hidden shadow-sm">
                  <div className="flex flex-col items-center gap-4">
                    ...
                    <p className="text-[13px] font-medium text-text-secondary">
                      {remaining > 0 ? (
                        <>Add items worth <span className="font-bold text-[#a9cb5b]">{formatPrice(remaining)}</span> for <span className="font-bold uppercase tracking-wider text-[#1a3014]">FREE Delivery!</span></>
                      ) : (
                ...
                <div className="bg-[#f8f9fa] border border-border rounded-xl p-6 sm:p-8 sticky top-24">
                  ...
                  <div className="space-y-6">
                    <div className="flex justify-between items-center text-sm">
                      <span className="font-bold font-jost uppercase tracking-wider text-text-secondary">Subtotal</span>
                      <span className="font-bold text-foreground text-lg">{formatPrice(total)}</span>
                    </div>

                    <div className="border-t border-border pt-6">
                      <span className="block font-bold font-jost uppercase tracking-wider text-xs text-text-secondary mb-4">Shipping</span>
                      <div className="space-y-4">
                        <label className="flex items-center justify-between cursor-pointer group">
                          <div className="flex items-center gap-3">
                            ...
                            />
                            <span className="text-sm text-foreground group-hover:text-[#a9cb5b] transition-colors">Flat rate</span>
                          </div>
                        ...
                        <label className="flex items-center justify-between cursor-pointer group">
                          <div className="flex items-center gap-3">
                            <input 
                              ...
                              className="w-4 h-4 accent-[#a9cb5b]"
                            />
                            <span className="text-sm text-foreground group-hover:text-[#a9cb5b] transition-colors">Local pickup</span>
                          </div>
                          <span className="text-sm font-bold text-text-secondary">Free</span>
                        </label>
                      </div>
                      <p className="text-[11px] text-text-secondary mt-4 leading-relaxed">
                        Shipping to <span className="font-bold text-foreground">United Kingdom</span>. 
                        <button 
                      ...
                      {showAddressForm && (
                        <div className="mt-6 space-y-4 pt-6 border-t border-border-light animate-in fade-in slide-in-from-top-2 duration-300">
                          <div className="space-y-1">
                            <label className="text-[10px] font-bold uppercase tracking-wider text-text-secondary">Country / region</label>
                          ...
                          <div className="space-y-1">
                            <label className="text-[10px] font-bold uppercase tracking-wider text-text-secondary">State / County <span className="text-red-500">*</span></label>
                            <select className="w-full h-11 px-3 border border-border rounded bg-white text-sm focus:outline-none focus:border-[#a9cb5b]">
                          ...
                          <div className="space-y-1">
                            <label className="text-[10px] font-bold uppercase tracking-wider text-text-secondary">City <span className="text-red-500">*</span></label>
                            <input type="text" className="w-full h-11 px-3 border border-border rounded bg-white text-sm focus:outline-none focus:border-[#a9cb5b]" />
                          </div>
                          <div className="space-y-1">
                            <label className="text-[10px] font-bold uppercase tracking-wider text-text-secondary">Postcode / ZIP <span className="text-red-500">*</span></label>
                            <input type="text" className="w-full h-11 px-3 border border-border rounded bg-white text-sm focus:outline-none focus:border-[#a9cb5b]" />
                          </div>
                    ...
                    <div className="border-t-2 border-border pt-6 flex justify-between items-center">
                      <span className="text-lg font-bold font-jost uppercase tracking-widest text-[#1a3014]">Total</span>
                      <span className="text-2xl font-bold text-[#a9cb5b]">{formatPrice(total + shippingCost)}</span>
                    </div>

                    <Link href="/checkout" className="w-full py-5 bg-[#a9cb5b] text-white font-bold rounded uppercase tracking-widest text-sm hover:opacity-95 transition-all shadow-lg shadow-[#a9cb5b]/20 flex items-center justify-center gap-2">
                      Proceed to Checkout
                    </Link>
        ...
        <section className="py-20 border-t border-border bg-[#f8f9fa]">
          <div className="max-w-[90%] mx-auto px-4 sm:px-6 lg:px-8">
            <div className="text-center mb-12">
              <h2 className="text-3xl font-bold font-jost uppercase tracking-widest mb-4 text-[#1a3014]">You may be interested in...</h2>
