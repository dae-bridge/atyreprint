    ...
    <>
      ...
      >
        ...
        <div className="flex items-center justify-between p-5 border-b border-border">
          <h2 className="text-xl font-bold font-jost text-foreground">Shopping Cart</h2>
          <button
            onClick={onClose}
          ...
          <div className="flex items-center justify-between mb-3">
             <div className="flex items-center gap-2">
                <div className={cn(
                  ...
                  progressPercent >= 100 ? "bg-green-500 border-green-500" : "bg-white border-[#ffda15]"
                )}>
                  <Truck size={18} className={progressPercent >= 100 ? "text-white" : "text-[#ffda15]"} />
                </div>
                <p className="text-sm font-medium text-foreground">
                  {amountToFreeShipping > 0 ? (
                    <>Add items worth <span className="font-bold text-[#a9cb5b]">£{amountToFreeShipping.toFixed(2)}</span> for <span className="font-bold">FREE Delivery!</span></>
                  ) : (
                    <span className="font-bold text-green-600">You've unlocked FREE Delivery!</span>
                  )}
                </p>
             </div>
          </div>
        ...
        <div className="flex-1 overflow-y-auto p-5 custom-scrollbar">
          {items.length === 0 ? (
            <div className="h-full flex flex-col items-center justify-center text-center py-10">
              ...
              <p className="text-text-muted mb-8 max-w-[250px]">Looks like you haven't added anything to your cart yet.</p>
              <button
                onClick={onClose}
            ...
            <div className="space-y-6">
              {items.map((item) => (
                <div key={`${item.productId}-${item.color}-${JSON.stringify(item.variants)}`} className="flex gap-4 group">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex justify-between gap-2 mb-1">
                      <h3 className="text-[15px] font-bold text-foreground hover:text-[#a9cb5b] transition-colors line-clamp-2 leading-tight">
                        <Link href={`/shop/product/${item.slug}`} onClick={onClose}>
                          {item.name}
                        </Link>
                      </h3>
                      <button 
        ...
        {items.length > 0 && (
          <div className="p-5 bg-white border-t border-border shadow-[0_-10px_30px_rgba(0,0,0,0.05)]">
            <div className="flex items-center justify-between mb-6">
              <span className="text-lg font-bold font-jost uppercase tracking-wider">Subtotal:</span>
              <span className="text-xl font-bold text-[#a9cb5b]">£{total.toFixed(2)}</span>
            </div>
